...
def synthesis(text, args):
    ...
    with torch.no_grad():
        for i in pbar:
            ...
            mel_pred, postnet_pred, _, stop_pred, _, _ = MODEL.forward(text, mel_input, pos_text, pos_mel)
            mel_input = torch.cat([mel_input, mel_pred[:, -1:, :]], dim=1)
    print(torch.max(torch.sigmoid(stop_pred)))
    ...
    torch.save(postnet_pred, 'ritesh_130k_postnet_mel.pt')
